...
syn keyword CSbasicCommands     alias at case casef cases clearsleep complete concat concatword cond conout conskip do echo error escape exec exists filter format history if indexof inputcommand keymap listclients listcomplete listdel listfind listlen listsplice local loop loopconcat loopconcatword loopfiles looplist loopwhile nodebug onrelease precf prettylist push quit resetvar result saycommand searchbinds searcheditbinds searchspecbinds setcomplete setdesc setinfo setpersist setpriv shrinklist sleep strcasecmp strcmp strlen strncasecmp strncmp strreplace strstr sublist substr toggleconsole while worldalias writecfg writeobj writevars
syn keyword CSbasicGetCommands  getalias getclienthost getclientname getclientnum getclientteam getcolour getfps getfvarmin getfvarmax getmillis getmodel getname getstate getteam getteamcolour getteamicon gettime getvar getvardef getvarflags getvariable getvarmin getvarmax getvartype getversion
syn keyword CSbindCommands      bind editbind specbind waitbind getbind geteditbind getspecbind getwaitbind
syn keyword CSmathCommands      acos asin atan cos div divf exp log2 log10 loge max maxf min minf mod modf pow rnd sin sqrt tan
...
syn keyword CSotherGetCommands  getloadweap getmaplist getpointer getserver getvote getweap
syn keyword CSguiCommands       guibackground
